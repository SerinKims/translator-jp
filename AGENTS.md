...
이 프로젝트는 로컬 LiteRT-LM의 `gemma4-e4b` 모델을 사용하여 사용자가 입력한 일본어 원문 또는 pixiv 소설 URL의 원문을 한국어 웹소설 문체로 번역하는 웹사이트이다.
...
4. **Frontend에서 pixiv 또는 LiteRT-LM을 직접 호출하지 않는다.**

   - pixiv 수집과 LiteRT-LM 호출은 Backend를 통해서만 수행한다.
