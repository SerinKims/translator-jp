...
기본값은 conda 환경 `tr-jp`입니다. `tr-jp`가 없으면 `python=3.11`과 `pip`를 포함해 새로 생성합니다. 이후 `backend/requirements.txt`를 기준으로 설치되지 않은 Python 패키지만 `tr-jp` 안에 설치합니다. base 환경에는 설치하지 않습니다.
