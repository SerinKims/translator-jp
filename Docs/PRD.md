...
- 로컬 LLM 기반으로 동작한다.
- 기본 모델은 `gemma4:26b-a4b-it-q4_K_M`이다.
- Ollama Python API를 통해 모델을 호출한다.
...
로컬에서 gemma4:26b-a4b-it-q4_K_M로 일본어 → 한국어 번역이 가능하다.
...
/api/health로 Ollama 실행 준비 상태와 DB 연결 상태를 확인할 수 있다.
...
## 2026-07-01 다국어 원문 번역 확장

MVP 번역 원문 언어는 일본어, 중국어 간체, 중국어 번체, 영어를 지원한다.
목표 언어는 기존과 동일하게 한국어만 지원한다.
...
- `source_lang=auto`이면 Backend에서 원문 언어를 감지한다.
- 히라가나/가타카나가 있으면 일본어를 우선한다.
- CJK 문자가 있고 히라가나/가타카나가 없으면 중국어로 감지한다.
- 번체 전용 문자가 뚜렷하면 `zh-TW`, 그 외 중국어는 `zh-CN`으로 처리한다.
- 알파벳 비중이 높고 CJK 문자가 거의 없으면 영어로 감지한다.
- 감지 실패 시 사용자가 언어를 직접 선택하도록 오류를 반환한다.
- `target_lang`은 MVP에서 `ko`만 허용한다.
